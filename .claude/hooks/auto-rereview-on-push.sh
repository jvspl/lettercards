...
has_open_findings=$(echo "$pr_json" | jq --arg op "$operator" '[.[0].comments[] | select(.author.login == $op and (.body | contains("🤖 Claude")) and (.body | contains("🔄")))] | length' 2>/dev/null)
[ "$has_open_findings" -gt 0 ] || exit 0

msg="Fixes were just pushed to PR #${pr_number}, which has an outstanding 🔄 review. Run a re-review (/pr-review ${pr_number}) if the findings are addressed, or confirm with the operator first."
